...
namespace Infosys.Ainauto.ConfigurationManager.BusinessComponent
{
    public class ResourceModelBuilder
    {
        ...
        public BE.PlatformInfo getActiveResource(string PlatformInstanceId, string TenantId, string ResourceTypeName)
        {
            ...
            try
            {
                ...
                var main = (from rdm in rdmDS.GetAll()
                            ...
                            select new { rdm.ResourceId, rdm.DependencyResourceId, rdm.DependencyType, res.ResourceName, resType.IsMainEntiry, resType.ResourceTypeName,
                ...
                var childResources = (from rdm in rdmDS.GetAll()
                                      ...
                                      select new { rdm.ResourceId, rdm.DependencyResourceId, res.ResourceName, resType.ResourceTypeId, resType.ResourceTypeName
                                                , res.ValidityStart, res.ValidityEnd, res.IsActive, rdm.PortfolioId,
                                          res.Source
                                      })
                                         .ToList();
            ...
            try
            {
                ...
                if (platforms != null)
                {
                    ...
                    typeConfiguration.resourcetypedetails = (from r in resDS.GetAny().ToArray()
                                                             ...
                                                             group new { rt, r } by new { rt.ResourceTypeId, rt.ResourceTypeName } into g
                            ...
                            foreach (BE.ResourceAttribute resAttr in resDetails.resourceattribute)
                            {
                                ...
                                if (entityItem != null)
                                {
                                    resource_Attributes.Add(new resource_attributes()
                                    {
                                        ...
                                        AttributeValue =resAttr.IsSecret && resAttr.attributename.ToLower()== "password"?Encrypt(resAttr.attributevalue):resAttr.attributevalue,
                                        ...
                                        foreach (BE.ResourceAttribute childResAttr in childRes.resourceattribute)
                                        {
                                            ...
                                            if (entityItem != null)
                                            {
                                                resource_Attributes.Add(new resource_attributes()
                                                {
                                                    ...
                                                    AttributeValue = childResAttr.IsSecret && childResAttr.attributename.ToLower() == "password" ? Encrypt(childResAttr.attributevalue) : childResAttr.attributevalue,
        ...
        private string Encrypt(string text)
        {
            ...
            FacadeClient client = new FacadeClient();
            return client.EncryptText(baseURI, "/" + endPoint + "/SecureHandler/EncryptData?textToSecure=" + text+"&passCode=IAP2GO_SEC!URE");
        ...
        private string Decrypt(string text)
        {
            ...
            string baseURI = baseURL.Split('/')[0];
            FacadeClient client = new FacadeClient();
            //return client.EncryptText("localhost", "superbot/api/SecureHandler/EncryptData?textToSecure=Infosys@123&passCode=IAP2GO_SEC!URE");
            return client.DecryptText(baseURI, "/"+endPoint+"/SecureHandler/DecryptText?secureText="+text+"&passCode=IAP2GO_SEC!URE");
        }
    }
