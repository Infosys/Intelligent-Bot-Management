...
{
    public class NotificationDS : IQueue<Notification>
    {
        ///<summary>
        ...
        public string Send(Notification message, string process)
        {
            using (LogHandler.TraceOperations("NotificationDS:Send",
                LogHandler.Layer.Resource, Guid.NewGuid(), null))
            {
                ...
                if (process == "DEFAULT" || process == "" || process == null)
                {
                    ...
                    msgResponse = adapterManager.Execute(serializedPresentationMsg, "NotificationHandler");
                    LogHandler.ArchiveMessages(serializedPresentationMsg, "NotificationHandler");
                    //LogHandler.LogDebug("MetricProcessorDS: Notification Payload for metric processor successfully posted.",
