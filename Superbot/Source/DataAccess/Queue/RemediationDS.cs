...
{
    public class RemediationDS : IQueue<Anomaly>
    {
        ///<summary>
        ...
        public string Send(Anomaly message, string process)
        {
            using (LogHandler.TraceOperations("RemediationDS:Send",
                LogHandler.Layer.Resource, Guid.NewGuid(), null))
            {
                ...
                if (process == "DEFAULT" || process == "" || process == null)
                {
                    ...
                    msgResponse = adapterManager.Execute(serializedPresentationMsg, "RemediationProcessor");
                    LogHandler.ArchiveMessages(serializedPresentationMsg, "RemediationProcessor");
                    //LogHandler.LogDebug("RemediationDS: Remediation Payload for metric processor successfully posted.",
